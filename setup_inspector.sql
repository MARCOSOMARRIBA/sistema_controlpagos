-- 1. Agregar columna solo_inspecciones a aseguradora
ALTER TABLE aseguradora ADD COLUMN IF NOT EXISTS solo_inspecciones BOOLEAN DEFAULT FALSE;

-- 2. Marcar BBVA como aseguradora de solo inspecciones
UPDATE aseguradora SET solo_inspecciones = TRUE WHERE UPPER(nombre) LIKE '%BBVA%';

-- 3. Recrear la funcion de corte mensual para incluir inspecciones sin factura
...
DECLARE
    ...
    v_total_inspecciones DECIMAL(12,2) := 0;
...
BEGIN
    ...
    -- Honorarios de facturas PAGADAS del ajustador en el mes
    ...
    -- Inspecciones sin factura (aseguradoras solo_inspecciones como BBVA)
    SELECT COALESCE(SUM(i.total_pagar), 0) INTO v_total_inspecciones
    FROM inspeccion i
    JOIN siniestro s ON i.id_siniestro = s.id_siniestro
    JOIN aseguradora a ON s.id_aseguradora = a.id_aseguradora
    WHERE s.id_ajustador = v_ajustador
      AND a.solo_inspecciones = TRUE
      AND to_char(i.fecha_inspeccion, 'YYYY-MM') = v_mes_corte;
    ...
    v_monto_neto := (v_total_honorarios + v_total_inspecciones) - v_total_anticipos;

    INSERT INTO corte_mensual (id_ajustador, mes_corte, total_honorarios, total_anticipos_descontados, monto_neto_pagado)
    VALUES (v_ajustador, v_mes_corte, v_total_honorarios + v_total_inspecciones, v_total_anticipos, v_monto_neto)
